...
public class FileUtils {
	 ...
	 */
	public static Entry getEntryForResource(Resource resource, String filesPath, URI baseUri) throws RegistryException {
		...
		String parentPath = relPath.isEmpty() ? null : relPath.substring(0, relPath.lastIndexOf("/") + 1);
	 ...
	 * @throws RegistryException Thrown if an error occurs when accessing the registry
	 */
	public static Entry treeWalk(String filesPath, String path, UserRegistry registry, URI baseUri, boolean recursive) throws RegistryException {
	 ...
	 * @throws RegistryException Thrown if an error occurs when accessing the registry
	 */
	public static Entry treeWalk(String filesPath, String path, Pattern fileNamePattern, Pattern mediaTypePattern, UserRegistry registry, URI baseUri, boolean recursive) throws RegistryException {
		...
		if (resource instanceof Collection) {
			...
			for (String child : folder.getChildren()) {
				...
				if (recursive)
					childEntry = treeWalk(filesPath, child, fileNamePattern, mediaTypePattern, registry, baseUri, recursive);
				else {
			...
			if (!filterMatches(entry, fileNamePattern, mediaTypePattern))
				entry = null;
	 ...
	 */
	public static boolean filterMatches(Entry entry, Pattern fileNamePattern, Pattern mediaTypePattern) {
		return filterMatches(entry.getName(), entry.getContentType(), fileNamePattern, mediaTypePattern);
	}
	 ...
	 */
	public static boolean filterMatches(String fileName, String mediaType, Pattern fileNamePattern, Pattern mediaTypePattern) {
