...
public class WorksetAPIImpl implements WorksetAPI {
	...
	public WorksetAPIImpl(String worksetId, UserRegistry registry, RegistryExtension registryExtension) throws RegistryException {
	...
	public Response getWorkset(@QueryParam("author") String author) {
		Log.debug(String.format("getWorkset: id=%s, author=%s, user=%s", _worksetId, author, _userName));
	...
	public Response updateWorkset(Workset workset, @DefaultValue("false") @QueryParam("public") boolean isPublic) {
		Log.debug(String.format("updateWorkset: id=%s, public=%s, user=%s", _worksetId, isPublic, _userName));
		...
		if (!_worksetId.equalsIgnoreCase(worksetMeta.getName()))
			Log.warn(String.format("API call for workset name '%s' does not match payload workset name '%s'", _worksetId, worksetMeta.getName()));
