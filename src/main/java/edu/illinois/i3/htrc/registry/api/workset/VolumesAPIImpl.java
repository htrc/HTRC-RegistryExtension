...
public class VolumesAPIImpl implements VolumesAPI {
    ...
    public VolumesAPIImpl(String worksetId, UserRegistry registry, RegistryExtension registryExtension) {
    ...
    public Response getVolumesAsPlainText(@QueryParam("author") String author) {
        Log.debug(String.format("getVolumesAsPlainText: id=%s, author=%s, user=%s", _worksetId, author, _userName));
            ...
            if (volumes != null)
                for (Volume volume : volumes.getVolumes()) {
                    ...
                    for (Property prop : volume.getProperties()) {
                        if (prop.getName().equals(Constants.HTRC_VOL_PROP_EPR)) {
    ...
    public Response getVolumesAsCSV(@QueryParam("author") String author) {
        Log.debug(String.format("getVolumesAsCSV: id=%s, author=%s, user=%s", _worksetId, author, _userName));
        ...
        try {
            ...
            if (volumes != null) {
                ...
                try {
                    ...
                    // set up the header
                    header[0] = Constants.HTRC_VOL_PROP_ID;
                    ...
                    for (Volume volume : volumes.getVolumes()) {
                        ...
                        // add the volume id
                        rowData.put(Constants.HTRC_VOL_PROP_ID, volume.getId());

                        // add the volume properties
                        for (Property property : volume.getProperties()) {
                            Log.debug(String.format("getVolumesAsCSV: vol: %s propName: %s propValue: %s",
    ...
    public Response getVolumes(@QueryParam("author") String author) {
        Log.debug(String.format("getVolumes: id=%s, author=%s, user=%s", _worksetId, author, _userName));
    ...
    public Response replaceVolumes(Volumes volumes) {
        ...
        try {
            ...
            Resource resource = _registry.get(resPath);
            resource.setContentStream(WorksetUtils.createWorksetContentStream(volumes.getVolumes()));
            resource.setProperty(Constants.HTRC_PROP_VOLCOUNT, Integer.toString(volumes.getVolumes().size()));
